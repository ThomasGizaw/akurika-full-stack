...
function OrderCom() {
  ...
  const liveTrack = async () => {
    await axios
      .get(
        `${process.env.NEXT_PUBLIC_BASE_URL}api/live-track-order?order_id=${id}`
      )
      .then((res) => {
        if (res?.data) {
          setDeliverymanLocation({
            lat: Number(res?.data?.data?.deliveryman_latitude),
            lng: Number(res?.data?.data?.deliveryman_longitude),
          });
        }
      })
      .catch((err) => {
      ...
      if (!resData) {
        axios
          ...
          .then((res) => {
            ...
            const checkDeliveryMan =
              Number(webSettings?.map_status) === 1 &&
              Number(res?.data?.order?.order_status) === 2 &&
              res?.data?.order?.deliveryman;

            if (checkDeliveryMan) {
              setDeliverymanLocation({
                lat: Number(res?.data?.order?.deliveryman?.latitude),
                lng: Number(res?.data?.order?.deliveryman?.longitude),
              });
            }
            const status = () => {
              switch (
  ...
  useEffect(() => {
    if (resData && resData?.deliveryman) {
      liveTrack();
    }
  }, [resData]);
  useEffect(() => {
    let interval;
    if (Number(webSettings?.map_status) === 1 && resData?.deliveryman) {
                  ...
                  resData?.deliveryman && (
                    <div className="w-full print:hidden block mt-5">
                      <MapShow
                        ...
                        destination={deliverymanLocation}
