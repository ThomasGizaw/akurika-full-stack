...
function CheakoutPage() {
  ...
  const getAllAddress = () => {
    axios
      ...
      .then((res) => {
        ...
        if (res.data && Number(webSettings?.map_status) === 1) {
          const firstAddress = res.data.addresses[0];
          const calcPrice = parseInt(
            firstAddress?.distance_in_km * firstAddress.per_km_price_range
          ).toFixed(2);
  ...
  const requestSaveAddressHandler = async (data) => {
    await apiRequest
      .saveAddress(auth().access_token, data)
      .then((res) => {
        setLoading(false);
        resetFormHandler();
        getAllAddress();
  ...
  const saveAddress = async () => {
    ...
    if (auth()) {
      ...
      if (Number(webSettings?.map_status) === 1) {
        if (location) {
          // api request
          requestSaveAddressHandler(data);
        } else {
          setLoading(false);
  ...
  const shippingHandler = (addressId, cityId) => {
    ...
    if (Number(webSettings?.map_status) === 1) {
      const findAddress =
        addresses &&
        addresses.length > 0 &&
        addresses.find((f) => parseInt(f.id) === addressId);
      const calcPrice = parseInt(
        findAddress?.distance_in_km * findAddress.per_km_price_range
      ).toFixed(2);
      setLocationShippingPrice(calcPrice);
    } else {
      const getRules =
      ...
      if (selectedBilling && selectedShipping) {
        if (Number(webSettings?.map_status) === 1) {
          const findAddress =
            addresses &&
            addresses.length > 0 &&
            addresses.find((f) => parseInt(f.id) === selectedShipping);
                  ...
                  <div className="w-full px-10 py-[30px] border border-[#EDEDED]">
                        ...
                        <div>
                          {locationShippingPrice && (
                            <div className="flex justify-between items-center">
                              ...
                              >
                                <CurrencyConvert
                                  price={Number(locationShippingPrice)}
                                />
                              </span>
                            </div>
                          )}
                    ...
                    <div className="mt-[30px]">
                      <div className=" flex justify-between mb-5">
                        ...
                        >
                          <CurrencyConvert
                            price={mainTotalPrice - discountCoupon}
                          />
                        </p>
                      </div>
                    </div>
